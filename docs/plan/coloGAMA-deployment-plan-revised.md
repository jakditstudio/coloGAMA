...
4. Frontend shows a friendly message if the backend is unreachable or errors out
5. Drop `sudo`, use proper Linux group permissions instead
6. Build frontend for production (static files, not dev server)
7. nginx serves the frontend + reverse-proxies `/api/` to FastAPI
8. Backend runs as a systemd service (auto-start, auto-restart)
9. Remove old crontab-based auto-start

**Why nginx, specifically:** it decouples "serving the page" from "running the backend." If the FastAPI process crashes or is mid-restart, nginx keeps serving the frontend regardless — the site stays reachable, and the frontend (step 4) can show a clear, human-readable message instead of the page going blank or failing to load at all. That separation of failure is the actual reason for nginx here, not raw performance.
...
## Step 3.5 — Show a friendly message when the backend is unreachable

This is the payoff for using nginx: the page itself stays up even if the backend is down, restarting, or errors out — so the frontend should catch that case and tell the user something they can actually understand, instead of a blank screen or a raw browser network error.

**Create a small shared helper**, e.g. `frontend/src/api.js`:

```javascript
const BACKEND_DOWN_MESSAGE =
  "The camera service isn't responding right now. It may be restarting — please wait a few seconds and try again.";
...
export async function apiRequest(path, options = {}) {
  ...
  if (!response.ok) {
    // Backend responded, but with an error (e.g. camera failure, 500)
...
**Use it in place of raw `fetch` calls**, e.g. in the Capture button's click handler:
...
async function handleCapture() {
  ...
  setLoading(true);
  try {
    const response = await apiRequest("/api/capture", { method: "POST" });
    const data = await response.json();
    // ...use data as before (pdf_url, captures, etc.)
  } catch (err) {
    setError(err.message); // shown in the UI below
  } finally {
    setLoading(false);
  }
}
...
**Display `error` somewhere visible in the component**, e.g.:
...
Apply the same `apiRequest` wrapper to the History page's fetch call too, so a backend hiccup there shows the same kind of friendly message instead of an empty/broken list.

**What this achieves in practice:** if the backend service is restarting (systemd's `Restart=on-failure` kicking in) or has crashed, a user opening or refreshing the page still sees the full app — header, layout, buttons — and gets a plain-language explanation instead of a dead page. This is the concrete, visible version of the reliability nginx provides underneath.
...
| Page loads but shows "camera service isn't responding" | Backend is down/restarting — check `sudo systemctl status cologama-backend` |
...
- **The site itself stays reachable even if the backend crashes or is mid-restart** — nginx keeps serving the frontend independently, and the frontend shows a plain-language message instead of failing silently
